...
export function handleNFTEvent(event: NFTEventEvent): void {
  ...
  // Update owner of NFT
  let nft = NFT.load(event.params.cAddress.toHex() + event.params.tokenId.toHex())
...
export function handleBidPlaced(event: BidPlacedEvent): void {
  let entity = new Bid(
    event.params.cAddress.toHex() + event.params.tokenId.toHex() + event.params.from.toHex()
  )
  entity.cAddress = event.params.cAddress
  entity.tokenId = event.params.tokenId.toI32()
  entity.from = event.params.from
...
export function handleBidCanceled(event: BidCanceledEvent): void {
  let id = event.params.cAddress.toHex() + event.params.tokenId.toHex() + event.params.from.toHex()
